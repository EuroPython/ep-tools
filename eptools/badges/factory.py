...
from .data import badge_files, qrcode_color, pythonpower_svg
from .printer import merge_badge_svgfiles, fill_text_contact_badge, create_qrcode
...
class BadgeFactory(object):
    ...
    def _badge_filepath(self, contact, role, outdir, with_email=True, prefix="badge_ep2017"):
        ...
        badge_filename = fname_template.format(
            prefix=prefix,
            role=role,
            id=contact.id,
    ...
    @staticmethod
    def _simple_badge_path(contact, role, outdir, prefix="badge_ep2017"):
        """ Return the filepath to the corresponding svg file of the contact in the `outdir`
        folder.
        ...
        badge_filename = fname_template.format(prefix=prefix, role=role, id=contact.id)
        return op.join(outdir, badge_filename)
